...
// New Route to fetch Approved Wardens
router.get('/wardens/approved', isAdmin, async (req, res) => {
    try {
        // Assuming 'approved_at' is set when a warden is approved
        // In reality, you might use a 'status' field (e.g., status: 'APPROVED')
        const approvedWardens = await pool.query(
            "SELECT * FROM wardens WHERE approved_at IS NOT NULL ORDER BY approved_at DESC"
        );
        res.json(approvedWardens.rows);
    } catch (err) {
        console.error("Error fetching approved wardens:", err.message);
        res.status(500).json({ message: "Server error in fetching approved list" });
    }
});

// New Route to fetch Rejected Wardens
router.get('/wardens/rejected', isAdmin, async (req, res) => {
    try {
        // Assuming 'rejected_at' is set, or a status field is used.
        // If rejected records are DELETED, this route won't work unless they are archived.
        // Let's assume you track rejections by setting a 'status' field to 'REJECTED'
        const rejectedWardens = await pool.query(
            "SELECT * FROM wardens WHERE rejected_at IS NOT NULL ORDER BY rejected_at DESC"
        );
        res.json(rejectedWardens.rows);
    } catch (err) {
        console.error("Error fetching rejected wardens:", err.message);
        res.status(500).json({ message: "Server error in fetching rejected list" });
    }
});

// Update existing Approve route:
router.patch('/wardens/:username/approve', isAdmin, async (req, res) => {
    const { username } = req.params;
    try {
        const result = await pool.query(
            "UPDATE wardens SET is_pending = FALSE, approved_at = NOW(), rejected_at = NULL WHERE username = $1 RETURNING *",
            [username]
        );
        // ... (rest of the logic)
    } catch (err) { /* ... */ }
...
// Update existing Reject route:
// Since the HTML implies REJECT removes the record, let's keep it as is, 
// but if you want to SHOW rejected records, you must use UPDATE instead of DELETE.


// If you want to SHOW Rejected Wardens (RECOMMENDED for new feature)
router.patch('/wardens/:username/reject', isAdmin, async (req, res) => {
    const { username } = req.params;
    try {
        const result = await pool.query(
            "UPDATE wardens SET is_pending = FALSE, approved_at = NULL, rejected_at = NOW() WHERE username = $1 RETURNING *",
            [username]
        );
        if (result.rowCount === 0) {
            return res.status(404).json({ message: "Warden not found" });
        }
        res.json({ message: "Warden status updated to REJECTED" });
    } catch (err) {
        console.error("Error rejecting warden:", err.message);
        res.status(500).json({ message: "Server error during reject" });
    }
});

// If you stick to DELETING the record (as per original logic), the new '/wardens/rejected' route will only show archived data, not live database data.
